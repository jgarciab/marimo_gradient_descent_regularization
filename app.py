...
def selection_helpers(
    ...
    return coefficient_path_rows, evaluate_cv_grid, evaluate_lambda_grid
...
def s1_controls(mo):
    ...
    s1_steps = mo.ui.slider(0, 25, value=0, step=1, label="Gradient descent steps")
...
def s1_section(
    ...
    _ax_loss.plot(_path, [mse_1p(beta) for beta in _path], "o-", color=path_color, linewidth=1.6, markersize=3.8, clip_on=False)
    _ax_loss.scatter([_current_beta], [mse_1p(_current_beta)], color=chosen_color, s=62, zorder=6, clip_on=False)
    _ax_loss.axvline(1.0, color=cost_color, linestyle="--", linewidth=1.7)
    _ax_loss.set(xlabel=r"$\beta_1$", ylabel="MSE", xlim=(-0.5, 2.5), ylim=(-0.05, 8.6))
    _ax_loss.set_xticks([-0.5, 0.0, 1.0, 2.0, 2.5])
        ...
        [
            ...
            note_md(
                ...
                if _diverged
                    ...
                    "The green point marks the starting coefficient before any update."
                    if int(s1_steps.value) == 0
...
def s2_controls(mo):
    ...
    s2_steps = mo.ui.slider(0, 30, value=0, step=1, label="Gradient descent steps")
...
def s2_section(
    ...
    _levels = np.linspace(0.0, 15.0, 21)
    _contour_fill = _ax_contour.contourf(_beta0_mesh, _beta1_mesh, _mse_surface, levels=_levels, cmap="Blues_r", alpha=0.88, extend="max")
    _ax_contour.contour(_beta0_mesh, _beta1_mesh, _mse_surface, levels=_levels, colors="#FFFFFF", linewidths=0.34, alpha=0.62)
    ...
    _cbar.set_label("MSE (capped at 15)", fontsize=8)
...
def s4_section(
    ...
    _common_levels = np.linspace(0.0, 15.0, 51)
    ...
    _cbar = _fig.colorbar(_contour_fill, cax=_cbar_ax)
    _cbar.set_label("Surface value (capped at 15)", fontsize=8)
    _cbar.ax.tick_params(labelsize=7)
    _sidebar = sidebar(
